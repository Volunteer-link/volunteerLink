    ...
    const fetchFinanceData = async () => {
      try {
        ...
        if (eventRes.data?.code === 200) {
          const mapped = eventRes.data.data.items?.map((item: any) => ({
            eventName: item.name,
            money: item.money,
          }));
      ...
      <div className="space-y-8">
        ...
        <div className="bg-white p-6 rounded-2xl shadow-lg">
          <div className="text-lg font-semibold text-gray-700 mb-4">
            Tổng số tiền ủng hộ theo tháng
          </div>
        ...
        <div className="bg-white p-6 rounded-2xl shadow-lg">
          <div className="text-lg font-semibold text-gray-700 mb-4">
            Những sự kiện được ủng hộ nhiều nhất
          </div>
          <TopDonatedEventsChart chartData={eventDonationData} />{" "}
          {/* Sử dụng component top sự kiện donate */}
        </div>
      </div>
    </div>
  );
